...
from .collections import DEFAULT_COLLECTIONS_PATH, CollectionRegistry
...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    for rank, item in enumerate(results, start=1):
        meta = f"{item.collection_name}/{item.relative_path} | {item.section_path}"
...
def cmd_collection_add(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        f"root={collection.root_path} mask={collection.mask}"
    )
    return 0
...
def cmd_collection_list(args: argparse.Namespace) -> int:
    ...
    for item in collections:
        ...
        print(
            ...
            f"root={item.root_path} mask={item.mask} {default_flag}"
        )
    return 0
